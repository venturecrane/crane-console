...
async function scheduleList(api: CraneApi, input: ScheduleInput): Promise<ScheduleResult> {
  try {
...
async function scheduleItems(api: CraneApi): Promise<ScheduleResult> {
  try {
...
async function scheduleLinkCalendar(api: CraneApi, input: ScheduleInput): Promise<ScheduleResult> {
  if (!input.name) {
    return {
      success: false,
...
async function scheduleComplete(api: CraneApi, input: ScheduleInput): Promise<ScheduleResult> {
  if (!input.name) {
    return { success: false, message: 'Schedule item name is required for the complete action.' }
  }
  if (!input.result) {
    return {
      success: false,
...
async function schedulePlannedEvents(api: CraneApi, input: ScheduleInput): Promise<ScheduleResult> {
...
async function schedulePlannedEventCreate(
  api: CraneApi,
  input: ScheduleInput
): Promise<ScheduleResult> {
  if (!input.event_date || !input.venture || !input.title || !input.start_time || !input.end_time) {
    return {
      success: false,
      message:
  ...
  try {
    const eventType = input.type as 'planned' | 'actual' | 'cancelled' | undefined
...
async function schedulePlannedEventUpdate(
  api: CraneApi,
  input: ScheduleInput
): Promise<ScheduleResult> {
  if (!input.id) {
  ...
  try {
    const updates: Partial<
      Pick<
        import('../lib/crane-api.js').PlannedEvent,
        'type' | 'start_time' | 'end_time' | 'sync_status' | 'gcal_event_id'
      >
    > = {}
    if (input.type !== undefined) updates.type = input.type as 'planned' | 'actual' | 'cancelled'
    ...
    const event = await api.updatePlannedEvent(input.id, updates)
    return {
      success: true,
...
async function schedulePlannedEventsClear(
  api: CraneApi,
  input: ScheduleInput
): Promise<ScheduleResult> {
  if (!input.from) {
...
function formatSessionHistoryBlock(
  entry: import('../lib/crane-api.js').SessionHistoryEntry,
  block: import('../lib/crane-api.js').SessionHistoryEntry['blocks'][number]
): string {
  ...
  const detail =
    ...
    (block.issues?.[0] ? ' #' + String(block.issues[0]) : '')
  return `| ${entry.work_date} | ${entry.venture.toUpperCase()} | ${start} | ${end} | ${block.session_count} | ${detail} |\n`
}

async function scheduleSessionHistory(
  api: CraneApi,
  input: ScheduleInput
): Promise<ScheduleResult> {
  try {
    ...
    for (const entry of entries) {
      for (const block of entry.blocks) {
        totalBlocks++
        table += formatSessionHistoryBlock(entry, block)
      }
    }
    return {
      success: true,
...
export async function executeSchedule(input: ScheduleInput): Promise<ScheduleResult> {
  ...
  switch (input.action) {
    case 'list':
      return scheduleList(api, input)
    case 'items':
      return scheduleItems(api)
    case 'link-calendar':
      return scheduleLinkCalendar(api, input)
    case 'complete':
      return scheduleComplete(api, input)
    case 'planned-events':
      return schedulePlannedEvents(api, input)
    case 'planned-event-create':
      return schedulePlannedEventCreate(api, input)
    case 'planned-event-update':
      return schedulePlannedEventUpdate(api, input)
    case 'planned-events-clear':
      return schedulePlannedEventsClear(api, input)
    case 'session-history':
      return scheduleSessionHistory(api, input)
    default:
      return {
        success: false,
        message: `Unknown action: ${String(input.action)}. Use "list", "complete", "items", "link-calendar", "planned-events", "planned-event-create", "planned-event-update", "planned-events-clear", or "session-history".`,
      }
  }
}
