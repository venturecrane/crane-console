...
import { mockVentures, mockDocAuditComplete, mockDocAuditIncomplete } from '../__fixtures__/api-responses.js';
...
describe('doc-audit tool', () => {
  ...
  it('fix mode generates and uploads missing docs', async () => {
    ...
    vi.mocked(scanLocalRepos).mockReturnValue([
      { path: '/Users/test/dev/smd-console', name: 'smd-console', remote: 'git@github.com:smd-ventures/smd-console.git', org: 'smd-ventures', repoName: 'smd-console' },
    ]);
    ...
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ success: true, scope: 'smd', doc_name: 'smd-project-instructions.md', version: 1 }),
    });
