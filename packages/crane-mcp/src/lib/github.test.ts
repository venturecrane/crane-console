    ...
    it('fetches all 5 queues', async () => {
      ...
      vi.mocked(execSync).mockImplementation((cmd) => {
        ...
        if (cmdStr.includes('gh api')) {
          ...
          if (cmdStr.includes('status:ready')) return mockGhApiSearchOutput(mockIssueBreakdown.ready);
          if (cmdStr.includes('status:in-progress')) return mockGhApiSearchOutput(mockIssueBreakdown.in_progress);
          if (cmdStr.includes('status:blocked')) return mockGhApiSearchOutput(mockIssueBreakdown.blocked);
          if (cmdStr.includes('status:triage')) return mockGhApiSearchOutput(mockIssueBreakdown.triage);
          return '[]';
        }
        return '';
      });
