...
**Required at:** Tier 1+ (all web frontends)

Every web frontend ships as an installable PWA. This gives users Add to Home Screen, fullscreen standalone mode, and basic offline shell caching at zero distribution cost. Safari 26 defaults every Home Screen addition to standalone web app mode - we meet it halfway with proper manifests and service workers.

**Framework integration:**

| Framework | Package           | Config Location    |
| --------- | ----------------- | ------------------ |
| Next.js   | `@serwist/next`   | `next.config.ts`   |
| Astro     | `@vite-pwa/astro` | `astro.config.mjs` |

**Required assets (in `public/`):**

| Asset                  | Size    | Format | Notes                     |
| ---------------------- | ------- | ------ | ------------------------- |
| `icon-192.png`         | 192x192 | PNG    | Manifest icon             |
| `icon-512.png`         | 512x512 | PNG    | Manifest icon + splash    |
| `apple-touch-icon.png` | 180x180 | PNG    | iOS Home Screen           |
| `icon.svg`             | Any     | SVG    | Modern browsers, maskable |
| `favicon.ico`          | 32x32   | ICO    | Legacy fallback           |
...
```json
{
  ...
  "theme_color": "{venture brand color}",
  "icons": [
...
**Service worker strategy:** Precache the app shell (HTML, CSS, JS, fonts). Use network-first for API calls. The goal is a functional offline shell, not full offline mode (that comes later per-venture as needed).

**iOS meta tags (required in layout):**

```html
<meta name="apple-mobile-web-app-capable" content="yes" />
<meta name="apple-mobile-web-app-status-bar-style" content="default" />
<meta name="theme-color" content="{venture brand color}" />
<link rel="apple-touch-icon" href="/apple-touch-icon.png" />
```

**Naming convention:** Manifest generated by the PWA plugin at build time. Icons prefixed consistently: `icon-{size}.png`, `apple-touch-icon.png`.

**Verification:** After implementation, test on iOS Safari: Add to Home Screen, verify standalone mode launches, verify app icon renders correctly, verify offline shell loads when network is off.
...
| ------- | ---------- | ---- | ------- | ----- | ---------- | --- | ----- | ------------------------- |
| DC      | Design     | 1    | No      | Basic | No         | No  | Basic | PWA + go-live             |
