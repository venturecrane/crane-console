...
| Crane MCP integration | `~/.hermes/hermes-agent/model_tools.py` patched with `"tools.crane_tools"` entry; `~/.hermes/hermes-agent/tools/crane_tools.py` provides the crane tool handlers. Verify with `scripts/hermes-verify-patch.sh`.     |
