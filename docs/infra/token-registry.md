...
| ------------------------------------------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `GH_TOKEN`                                  | GitHub PAT (`repo`, `workflow`)                   | Infisical `prod:/vc` `GH_TOKEN`, then synced to all seven venture paths (`/vc /ss /ke /sc /dfg /dc /smd`)        | Rotated on 2026-08-19 (`crane-agent-token-2026-08`) after the prior PAT expired 2026-08-18 unnoticed. Live at all **seven** venture paths, scopes verified `repo, workflow`, HTTP 200 from each (`vfy_01M0DQ857BR1M4EKFT5V6CKFHJ`). **Expires 2026-11-17.**                                                                  | GitHub auth for `crane`-launched sessions and shared fleet tooling                 | `packages/crane-mcp`, `scripts/setup-claude-desktop-mcp.sh`, notifications and fleet scripts | [GH_TOKEN - Infisical shared PAT](token-rotation-runbook.md#gh_token-infisical-shared-pat)                       |
| `NODE_AUTH_TOKEN`                           | GitHub classic PAT (`read:packages`)              | Infisical `prod:/vc` `NODE_AUTH_TOKEN`, then synced to all seven venture paths (`/vc /ss /ke /sc /dfg /dc /smd`) | Rotated on 2026-08-19 (`enterprise-packages-read-2026-08`) after the prior PAT expired 2026-07-19 and went unnoticed for a month. Live at all **seven** venture paths, scope verified `read:packages` only, registry HTTP 200 and a real `npm install` succeeded (`vfy_01M0DQGBA8NFPGNJA9RB1EGPTP`). **Expires 2026-11-17.** | Pull private `@venturecrane/*` npm packages                                        | `npm install`, launcher-injected agent env, fleet machines                                   | [NODE_AUTH_TOKEN - GitHub Packages PAT](token-rotation-runbook.md#node_auth_token-github-packages-pat)           |
...
| `GITHUB_CLIENT_ID` + `GITHUB_CLIENT_SECRET` | GitHub OAuth App client pair                      | Infisical `/vc` (record only).                                                                                   | App `venturecrane-github` (ID `2619905`) — the App survives and backs `crane-watch`; only this OAuth client pair is retired.                                                                                                                                                                                                 | **Retired 2026-07-29** — no runtime consumer since `crane-mcp-remote` was removed. | n/a                                                                                          | n/a                                                                                                              |
...
### GitHub App `venturecrane-github` (ID `2619905`) — OAuth client pair

**Retired as a consumer surface on 2026-07-29.** The OAuth client pair
(`GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET`) existed to give claude.ai GitHub access through
`workers/crane-mcp-remote`. That worker was removed when the cloud MCP surfaces were retired in
favour of Remote Control; **no runtime consumes the OAuth client pair today.**

The **App itself survives and must not be deleted** — it is also the auth substrate for
`crane-watch` (PEM, webhook secret) across four installations. See
`docs/instructions/secrets.md`. The client pair remains in Infisical `/vc` as the record for that
App, not as a live credential.

The prior open action here — record the literal "Authorization callback URL" from the OAuth App
settings page — closes as **N/A**: there is no longer a worker presenting a `redirect_uri`.
