...
| `supersedes_source`   | string[] | Path(s) this memory was distilled from (retrospective .md, per-project memory file, issue). Auditable lineage. **Path-on-disk audited** by `crane_memory_audit` — every entry must exist as a file. Do **not** put `vfy_` ledger IDs here; use `evidence_verify_ids` instead.                                                                                                                                                                                                                                                               |
| `evidence_verify_ids` | string[] | Verify-ledger row IDs that motivated this memory. Populated by `crane_verify_audit --apply` when a recurring `(command_hash, repo)` pattern is nominated as a draft lesson. Each entry must match `/^vfy_[26 Crockford]$/`; the audit owns ledger-existence checks, save validates regex shape only. Rendered inline in `/memory-audit`'s `pending_captain_approval[]` so the Captain sees the lineage at approval time. **Do not conflate with `supersedes_source`** — that field is for source files on disk, this one is for ledger IDs. |
