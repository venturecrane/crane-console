---
sidebar:
  order: 4
---

# Roadmap

## Current Focus

- Define sprint deliverables and pricing
- Build intake questionnaire
- Create client-facing materials
- Draft MSA and SOW templates

## Near-term

- First pilot client (discounted/free for case study)
- Refine sprint playbook based on pilot
- Publish case study from pilot
- Launch paid offerings

## Future

- Productized sprint packages (tiered pricing)
- Self-service validation tools
- Partner/affiliate program
- Training for accelerator partners

## Completed

- VaaS framework design
- Core methodology documentation
- Manifesto and positioning
- Experiment stage definitions
- Engineering charter

## Dependencies

| Item         | Blocks                | Owner         |
| ------------ | --------------------- | ------------- |
| Client MSA   | First paid engagement | SMDurgan LLC  |
| SOW Template | Any sprint            | SMDurgan LLC  |
| Case study   | Marketing launch      | Silicon Crane |
