...
| **MCP / Model Context Protocol**                   | Open protocol for connecting Claude agents to tools, resources, and services. Anthropic introduced it Nov 2024; now an industry standard under the Agentic AI Foundation | Use when describing crane-mcp. Appropriate on infrastructure and methodology pages. **(2026-07-29: crane-mcp-remote was retired; do not reference it in new copy.)**                    | Treating MCP as equivalent to "using Claude." MCP is the protocol layer, not the model.                       |
...
| **claude.ai**                                      | Anthropic's web interface. **(2026-07-29: the crane-mcp-remote worker that served MCP to claude.ai was retired; cloud access is now Claude Code Remote Control.)**       | Do **not** claim a remote MCP capability for claude.ai in new copy — that surface no longer exists. Remote Control drives a live CLI session instead.                                   | Confusing claude.ai with Claude Code. The web interface and the CLI are different surfaces.                   |
