...
1. Search Google Calendar for an existing `{VENTURE_CODE} Session` event on that date to avoid duplicates
2. If not found, create a Google Calendar event:
   - **Title**: `{VENTURE_CODE} Session` (e.g., `VC Session`)
   - **Start**: `06:30` (America/Phoenix timezone)
   - **End**: `22:30` (America/Phoenix timezone)
3. After successful GCal creation, store in D1:
   - Call `crane_schedule(action: "planned-event-create", event_date: "{date}", venture: "{code}", title: "{VENTURE_CODE} Session", start_time: "06:30", end_time: "22:30", gcal_event_id: "{event_id}")`
