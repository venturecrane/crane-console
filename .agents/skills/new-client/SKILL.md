...
depends_on:
  files:
    ...
    - crane-console:workers/crane-context/src/endpoints/admin-provision-engagement.ts
    - crane-console:docs/process/new-engagement-setup-checklist.md
  commands:
    - jq
    - curl
    - git
...
If the venture entry has no `clients` array yet, the `//= []` initialization at the top of the jq pipeline creates it before append.
...
- **Engagement skill:** `.agents/skills/new-engagement/SKILL.md`
