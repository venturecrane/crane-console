...
- `.claude/commands/`, `.claude/agents/`, `.claude/settings.json` hooks
...
- Individual venture product codebases (vc-web, dc-marketing, dfg-console, sc-console, kidexpenses) — those are separate audits.
...
- `ls` of `workers/`, `packages/`, `scripts/`, `.claude/`, `docs/`, `~/.claude/skills/`
- Count slash commands, skills, migrations, scripts, workflows
...
| 2   | **Skills, commands, hooks** | Command overlap (lifecycle / review / editorial clusters), AGENTS.md vs GEMINI.md drift, settings.json deny/allow precedence, command bloat (commands >10KB), CLAUDE.md content that should be in fetched docs                                     |
...
### 6. Offer to act on findings

Ask the Captain whether to:

1. File the critical-list items as GitHub issues (with appropriate `qa-grade:*` labels)
...
Runs monthly via remote trigger. Manual invocation always allowed via `/platform-audit`.

## Rules

- All GitHub issues this session target the repo shown in context. Targeting a different repo? STOP.
- Never write the report directly to the previous audit's filename — always use today's date.
