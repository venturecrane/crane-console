...
export async function handleRegisterMachine(request: Request, env: Env): Promise<Response> {
...
export async function handleMachineHeartbeat(
  ...
  try {
    ...
    if (!machine) {
      return errorResponse('Machine not found', HTTP_STATUS.NOT_FOUND, context.correlationId, {
        id: machineId,
      })
    }
...
export async function handleSshMeshConfig(request: Request, env: Env): Promise<Response> {
  const context = await buildRequestContext(request, env)
  if (isResponse(context)) {
    return context
  }
