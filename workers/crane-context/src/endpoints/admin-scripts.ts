...
function validateUploadScriptBody(body: UploadScriptRequest): Response | null {
...
async function upsertScript(env: Env, opts: UpsertScriptOptions): Promise<Response> {
  const { body, contentHash, contentSizeBytes, scriptType, executable, now } = opts
...
export async function handleUploadScript(request: Request, env: Env): Promise<Response> {
  ...
  const validationError = validateUploadScriptBody(body)
