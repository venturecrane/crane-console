/**
 * Integration Tests: POST /sos (Start of Session)
 *
...
describe('POST /sos - Session Resume Logic', () => {
  // ============================================================================
  ...
  it('creates new session when none exists', async () => {
    ...
    const response = await post('/sos', data)
  ...
  it('resumes active non-stale session', async () => {
    ...
    // Create session
    const createResponse = await post('/sos', data)
    ...
    const resumeResponse = await post('/sos', data)
  ...
  it('returns cached response for replayed idempotency key', async () => {
    ...
    const firstResponse = await post('/sos', {
      ...data,
    ...
    const firstResponseWithKey = await fetch('http://localhost:8787/sos', {
      method: 'POST',
      headers: {
    ...
    const replayResponse = await fetch('http://localhost:8787/sos', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Relay-Key': 'test-relay-key-for-integration-testing',
