...
function shouldSkipClassification(payload: GradeIssueRequest['payload']): {
  skip: boolean
  reason?: string
} {
...
function validateClassifyRequest(
  payload: any
): { ok: true; request: GradeIssueRequest } | { ok: false; message: string } {
...
async function handleClassify(
  ...
  if (semanticCheck) {
    ...
    return v2Json({
      ...
      task: 'grade_issue',
      idempotency_key: request.idempotency_key,
      result: semanticCheck.model_output_json
        ? JSON.parse(semanticCheck.model_output_json)
        : undefined,
      actions_taken: [
        {
          type: 'classify',
...
function jsonResponse(data: unknown, status = 200): Response {
  return new Response(JSON.stringify(data, null, 2), {
    ...
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': currentRequest
        ? getCorsOrigin(currentRequest)
        : ALLOWED_ORIGINS[0],
    },
  })
}
...
function validateEvent(e: any): { ok: true; event: RelayEvent } | { ok: false; message: string } {
  ...
  if (!event_id || event_id.length < 8)
    return { ok: false, message: 'event_id is required (min length 8)' }
  if (!repo || !isRepoSlug(repo)) return { ok: false, message: "repo must be 'org/repo'" }
  if (!issue_number || issue_number < 1)
    return { ok: false, message: 'issue_number must be a positive integer' }
  if (!['QA', 'DEV', 'PM', 'MENTOR'].includes(role))
    return { ok: false, message: 'role must be QA|DEV|PM|MENTOR' }
  ...
  if (e.build) {
    ...
    const commit_sha = String(e.build.commit_sha || '')
      .trim()
      .toLowerCase()
    const pr = e.build.pr != null ? safeInt(e.build.pr) : undefined
    if (!commit_sha || !isHexSha(commit_sha))
      return { ok: false, message: 'build.commit_sha must be a hex sha (7-40 chars)' }
  ...
  const overall_verdict: Verdict | undefined = e.overall_verdict
  if (
    overall_verdict &&
    !['PASS', 'FAIL', 'BLOCKED', 'PASS_UNVERIFIED', 'FAIL_UNCONFIRMED'].includes(overall_verdict)
  ) {
  ...
  if (e.scope_results != null) {
    ...
    for (const r of scope_results!) {
      ...
      if (!['PASS', 'FAIL', 'SKIPPED'].includes(r.status))
        return { ok: false, message: 'scope_results[].status must be PASS|FAIL|SKIPPED' }
    }
  }
  ...
  const evidence_urls = e.evidence_urls
    ? Array.isArray(e.evidence_urls)
      ? e.evidence_urls.map((u: any) => String(u))
      : null
    : undefined
  if (evidence_urls === null)
    return { ok: false, message: 'evidence_urls must be an array of strings' }
...
async function upsertRollingComment(
  env: Env,
  ghToken: string,
  repo: string,
  issueNumber: number,
  ...
  while (page <= 3 && !found) {
    ...
    found =
      comments.find((c) => typeof c.body === 'string' && c.body.includes(RELAY_STATUS_MARKER)) ||
      null
  ...
  if (found?.id) {
    ...
    await env.DB.prepare(
      ...
      .bind(repo, issueNumber, commentId, nowIso())
      .run()
    return commentId
  }
...
async function getLatestEventByType(
  ...
  issueNumber: number,
  eventType: string
): Promise<{ created_at: string; payload_json: string } | null> {
  const row = await env.DB.prepare(
    ...
    .bind(repo, issueNumber, eventType)
    .first<{ created_at: string; payload_json: string }>()
  return row || null
}
...
function renderRelayStatusMarkdown(input: {
  ...
  const prov =
    input.provenance.verified === null
      ? 'n/a'
      : input.provenance.verified
        ? 'VERIFIED (matches PR head)'
  ...
  const evidenceLines = qaEvidence.length ? qaEvidence.map((u) => `  - ${u}`).join('\n') : '  - n/a'

  const recentLines = input.recent.length
    ? input.recent
        .map((r) => `- ${r.created_at.slice(11, 16)}Z — ${r.event_type} — ${r.agent}`)
        .join('\n')
    : '- n/a'
...
async function handleEvidenceUpload(req: Request, env: Env): Promise<Response> {
  ...
  await env.DB.prepare(
    ...
    .bind(id, repo, issueNumber, eventId, filename, fileType, sizeBytes, r2Key, createdAt)
    .run()
...
async function handlePostEvents(
  ...
  // Idempotency check
  const existing = await env.DB.prepare('SELECT payload_hash FROM events WHERE event_id = ?')
    .bind(event.event_id)
  ...
  const latestQaRow = await getLatestEventByType(
    env,
    event.repo,
    event.issue_number,
    'qa.result_submitted'
...
async function handleApprove(
  ...
  for (const id of payload.ids) {
    try {
      ...
      const issue = await getIssueDetails(env, ghToken, item.repo, item.issue_number)
      const latestDevRow = await getLatestEventByType(
        env,
        item.repo,
        item.issue_number,
        'dev.update'
