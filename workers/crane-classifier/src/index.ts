...
async function githubFetch(token: string, method: string, path: string, body?: unknown): Promise<Response> {
...
async function getInstallationToken(env: Env, repo: string): Promise<string> {
  ...
  const res = await githubFetch(appJwt, "POST", `/app/installations/${installationId}/access_tokens`);
  if (!res.ok) {
...
async function callGeminiFlash(
  ...
  responseSchema: unknown,
  timeoutMs: number
): Promise<{ ok: boolean; result?: GradeIssueResult; raw?: string; error?: string; latency: number }> {
...
async function checkIdempotency(
  ...
  ).bind(idempotencyKey).first<{ grade?: string; confidence?: number }>();
...
async function classifyIssue(
  ...
  ).bind(semanticKey).first<{ grade?: string; confidence?: number }>();
...
async function handleGitHubWebhook(req: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return new Response(`OK - error: ${err instanceof Error ? err.message : String(err)}`, { status: 200 });
  }
}
